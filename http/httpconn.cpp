                     ...
                     int close_log, std::string user, std::string passwd, std::string sqlname)
{
    ...
    addfd(epollfd_, sockfd, true, TRIGMode);
    user_count_++;
...
bool http_conn::write()
{
    ...
    while (1)
    {
        temp = writev(sockfd_, iv, iv_count_);
        ...
        bytes_to_send_ -= temp;
        if (bytes_have_send_ >= iv[0].iov_len)
        {
            iv[0].iov_len = 0;
            iv[1].iov_base = file_address_ + (bytes_have_send_ - write_idx_);
            iv[1].iov_len = bytes_to_send_;
        }
        else
        {
            iv[0].iov_base = write_buf_ + bytes_have_send_;
            iv[0].iov_len = iv[0].iov_len - bytes_have_send_;
        }
...
bool http_conn::process_write(HTTP_CODE ret)
{
    ...
    case FILE_REQUEST:
    {
        ...
        if (file_stat_.st_size != 0)
        {
            ...
            iv[0].iov_base = write_buf_;
            iv[0].iov_len = write_idx_;
            iv[1].iov_base = file_address_;
            iv[1].iov_len = file_stat_.st_size;
            iv_count_ = 2;
